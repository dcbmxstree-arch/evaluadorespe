// grupo3.js - Lógica para el Grupo 3 (Álgebra + Química integrado)

// Unificar bancos de preguntas
const BANCO_GRUPO3 = (typeof GRUPO3_ALGEBRA_01 !== 'undefined' ? GRUPO3_ALGEBRA_01 : [])
  .concat(typeof GRUPO3_QUIMICA_01 !== 'undefined' ? GRUPO3_QUIMICA_01 : []);

// Configuración de modalidades (diagnóstico 30/90)
...
const REQUISITOS_DIAGNOSTICO = {
  "1.1 Proposiciones y conectivos lógicos": 1,
  "4.3 Cálculos estequiométricos a partir de ecuaciones químicas": 1,
  "5.2 Propiedades, ecuaciones e inecuaciones (exponenciales)": 1
};

// Estado global
const AppState = {
  estudiante: { nombre: 'Estudiante', notaColegio: 8.5, carrera: '', carreraId: 14 },
...
// Mapeo de carreras del Grupo 3 (IDs 14-17 según perfiles_ingreso.js)
const CARRERAS_GRUPO3 = {
  "Biotecnología": 14,
  "Medicina": 15,
  "Agropecuaria": 16,
  "Tecnología Superior en Aeronáutica (Piloto)": 17
};
...
function renderizarCronograma() {
  ...
  if (!container) return;
  let html = '<div class="bg-white rounded-xl shadow-md overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr><th class="px-4 py-2 text-left">Semana</th><th class="px-4 py-2 text-left">Temas (Álgebra + Química)</th></tr></thead><tbody>';
  for (let s = 1; s <= 10; s++) {
    const temas = CRONOGRAMA_GRUPO3[s] || [];
    html += `<tr class="border-b"><td class="px-4 py-2 font-medium">Semana ${s}</td><td class="px-4 py-2">${temas.join(', ')}</td></tr>`;
  }
  html += '</tbody>｜DSML｜</div></div>';
...
function iniciarSimulador() {
  ...
  AppState.estudiante.carreraId = CARRERAS_GRUPO3[carrera] || 14;
  ...
  const subtemas = obtenerSubtemasEvaluacionGrupo3(modalidad, semana);
  mostrarPrevisualizacion(subtemas, modalidad, semana);
}

function obtenerSubtemasEvaluacionGrupo3(modalidad, semana) {
  if (modalidad === 'diagnostico' || modalidad === 'final') {
    let todos = [];
    for (let i = 1; i <= 10; i++) todos = todos.concat(CRONOGRAMA_GRUPO3[i]);
    return [...new Set(todos)];
  }
  if (modalidad === 'semanal') return CRONOGRAMA_GRUPO3[semana] || [];
  if (modalidad === 'acumulado') {
    let acum = [];
    for (let i = 1; i <= semana; i++) acum = acum.concat(CRONOGRAMA_GRUPO3[i]);
    return [...new Set(acum)];
  }
  return [];
}
...
function comenzarExamen(subtemas) {
  ...
  let pool = BANCO_GRUPO3;
  if (modalidad === 'final') pool = pool.filter(p => p.dificultad === 'media' || p.dificultad === 'alta');
  else pool = filtrarPreguntas(pool, 3, AppState.estudiante.carreraId, subtemas);
  ...
  if (AppState.examenActual.length < AppState.config.numPreguntas) {
    alert(`Solo hay ${AppState.examenActual.length} preguntas disponibles. Se ajustará el número.`);
    AppState.config.numPreguntas = AppState.examenActual.length;
  }
...
function mostrarResultados(resultado) {
  ...
  if (!estado || estado.puntajeTotal === undefined) {
    document.getElementById('view-results').innerHTML = `<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">Error: No se pudo calcular el puntaje de ingreso. Verifique la carrera seleccionada.</div>`;
    return;
  }
  ...
  document.getElementById('btn-descargar')?.addEventListener('click', () => {
    const informe = {
      estudiante: { carrera: AppState.estudiante.carrera, notaColegio: AppState.estudiante.notaColegio },
      examen: {
    ...
    a.href = url;
    a.download = `evaluacion_grupo3_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
