// grupo2.js - Lógica para el Grupo 2 (Matemática Básica)

const BANCO_GRUPO2 = typeof GRUPO2_MATEMATICA_01 !== 'undefined' ? GRUPO2_MATEMATICA_01 : [];
...
const REQUISITOS_DIAGNOSTICO = {
  "1.1 Historia de los Sistemas de Numeración": 1,
  "2.5 Máximo común divisor y mínimo común múltiplo": 1,
  "7.3 Tanto por ciento": 1
};

// Estado global
const AppState = {
  estudiante: { nombre: 'Estudiante', notaColegio: 8.5, carrera: '', carreraId: 7 },
...
// Mapeo de carreras del Grupo 2 (IDs usados en las preguntas)
const CARRERAS_GRUPO2 = {
  "Educación Inicial": 7,
  "Pedagogía de la Actividad Física y Deporte": 8,
  "Educación Básica": 9,
  "Pedagogía del Idioma Inglés": 10,
  "Turismo": 11,
  "Tecnología Superior en Seguridad y Prevención de Riesgos Laborales": 12,
...
function renderizarCronograma() {
  ...
  if (!container) return;
  let html = '<div class="bg-white rounded-xl shadow-md overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr><th class="px-4 py-2 text-left">Semana</th><th class="px-4 py-2 text-left">Temas</th></tr></thead><tbody>';
  for (let s = 1; s <= 10; s++) {
    const temas = CRONOGRAMA_GRUPO2[s] || [];
...
function iniciarSimulador() {
  ...
  AppState.estudiante.carreraId = CARRERAS_GRUPO2[carrera] || 7;
  ...
  const subtemas = obtenerSubtemasEvaluacionGrupo2(modalidad, semana);
  mostrarPrevisualizacion(subtemas, modalidad, semana);
}

function obtenerSubtemasEvaluacionGrupo2(modalidad, semana) {
  if (modalidad === 'diagnostico' || modalidad === 'final') {
    let todos = [];
    for (let i = 1; i <= 10; i++) todos = todos.concat(CRONOGRAMA_GRUPO2[i]);
    return [...new Set(todos)];
  }
  if (modalidad === 'semanal') return CRONOGRAMA_GRUPO2[semana] || [];
  if (modalidad === 'acumulado') {
    let acum = [];
    for (let i = 1; i <= semana; i++) acum = acum.concat(CRONOGRAMA_GRUPO2[i]);
    return [...new Set(acum)];
  }
  return [];
}
...
function comenzarExamen(subtemas) {
  ...
  let pool = BANCO_GRUPO2;
  if (modalidad === 'final') pool = pool.filter(p => p.dificultad === 'media' || p.dificultad === 'alta');
  else pool = filtrarPreguntas(pool, 2, AppState.estudiante.carreraId, subtemas);

  // Generar examen (con requisitos opcionales para diagnóstico)
...
function mostrarResultados(resultado) {
  ...
  if (!estado || estado.puntajeTotal === undefined) {
    document.getElementById('view-results').innerHTML = `<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">Error: Carrera no reconocida.</div>`;
    return;
  }
  ...
  document.getElementById('btn-descargar')?.addEventListener('click', () => {
    ...
    a.href = url;
    a.download = `evaluacion_grupo2_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
